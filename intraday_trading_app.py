...
class IntradayTradingApp:
    ...
    def calculate_trading_plan(self, entry_price, lots):
        try:
            ...
            st.error(f"Error calculating trading plan: {str(e)}")
            st.error(traceback.format_exc())
            raise

def main():
    try:
        st.set_page_config(page_title="IntradayTrading", layout="wide")
        ...
        st.title("Loading Intraday Trading App...")
        
        # Initialize app
        app = IntradayTradingApp('panin-financ_full_dataset.json')
        ...
        with col1:
            ...
            **Market Cap:** Rp {app.company_info['market_cap']:,.2f}
            """)
